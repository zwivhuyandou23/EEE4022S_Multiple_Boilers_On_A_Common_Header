function q_f = feedwater_valve_boiler1(m)
...
K_vs        = 172.527;    % set ~100 for ~40 kg/s at m~0.75, DeltaP=0.5 MPa, T~290 C
...
Q_m3_hr = K_vs * max(0,min(1,m)) * sqrt( max(DeltaP_bar,0) / max((rho / 1000),eps) );

% Mass flow (kg/s)
q_f = (rho * Q_m3_hr) / 3600;
end